...
The omitted variable bias will only be zero if $x_2$ has no covariance with $x_1$ ($cov(x_1, x_2) = 0$) or if $\beta_2$ is 0. I know that. What I *don't* know is how to explain it based on the mess of formulae I just produced. 
...
```{r problem5b}
# Load and clean data
turnout <- read.csv("https://raw.githubusercontent.com/jnseawright/ps405/refs/heads/main/Data/turnout.csv")
...
Aha! Looking at the guide and my class notes, it seems I need to set some tolerance in `solve()`. This sets me up for takes four, five, and six, in which I tried to set tolerance and failed multiple times for unclear reasons. Ultimately I set the tolerance to zero and just saw what happened, and it worked, so I decided I was probably doing it backwards, and on take 8, set `tol=1e-40`. 


```{r problem5b_ols_coefs_matrix}
