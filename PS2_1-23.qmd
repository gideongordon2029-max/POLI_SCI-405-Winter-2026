...
However, the CEF, while it exists theoretically, may have no practical value at all. I can try to find a CEF for the relationship between [people with the first name Gerard and the air pollution in Alaska](https://www.tylervigen.com/spurious/correlation/2018_popularity-of-the-first-name-gerard_correlates-with_less-than-ideal-air-quality-in-anchorage). It won't mean anything about whether there's a real relationship, but it will exist. This also goes for the BLP.
...
$m$ here refers to an expected value (or a mean of values). $\mathbb{X}$ is the input for the function $m$, and represents a vector of $X$s of different values; the values of each element in $\mathbb{X}$ are given by the different $X$s on the right side of the formula. The $\beta$s are each a coefficient with which their respective $X$s are multiplied, with the exception of the intercept $\beta_0$. There can be any number of terms in this BLP added linearly together, until $\beta_k X_k$. The symbol $k$ is used to mark the last element in a vector, and can be any natural number (that is, any natural number higher than the rest of the elements). 
...
```{r}
model_summary = summary(blp_model)
...
More or less. As I noted looking at the LOESS curve in 4a, the curve itself seems to decline to the right, but we also know that this is a result of a few very wealthy authoritarian outliers. Democracy scores increase (though unevenly) until about $60,000 and then stop increasing. 
...
The BLP massively overpredicts democracy in very wealthy countries; the quadratic function seems to imply that democracy actually declines once a country gets "too wealthy". The log-linear model seems to intuitively capture the relationship best, but...
...
...when I did the math, I did find that the polynomial MSE had the lowest error. This I suppose is a cautionary tale in the difference between a model being useful for describing data precisely and a model being useful for making policy.  
...
1. It fits well enough without overcorrecting, and as I said for the BLP, it smooths out some of the outliers. 
