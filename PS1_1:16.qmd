...
state_ice_arrests = ice_arrests |>
  ...
  summarise(count = n())
state_ice_arrests = mutate(state_ice_arrests, arrests = count)
state_ice_arrests = mutate(state_ice_arrests, count = NULL)
```
...
```{r}
partisan_population_arrests = lm(arrests ~ Party + Population, data = state_ice_arrests_population)
...
```{r}
state_ice_arrests_population = mutate(state_ice_arrests_population, proportional_arrests = (10000*(arrests/Population)))
```
...
```{r}
partisan_proportional_arrests = lm(proportional_arrests ~ Party, data = state_ice_arrests_population)
